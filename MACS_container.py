...
Container_Num = 240
resource_total_EN =np.array([[64, np.random.randint(40,65), np.random.randint(300,500)]for i in range(10)])
...
np.savetxt("D:\ACS7\Resource_total_EN.txt", resource_total_EN, fmt='%d',delimiter=',')
np.savetxt("D:\ACS7\Resource_Container.txt", resource_Container,fmt='%d',delimiter=',')
np.savetxt("D:\ACS7\data_trans_Container.txt", data_trans_Container,fmt='%d',delimiter=',')
np.savetxt("D:\ACS7\Band.txt", Band,fmt='%d',delimiter=',')
np.savetxt("D:\ACS7\X.txt", X,fmt='%d',delimiter=',')
Y = np.copy(X)
print('部署决策矩阵：\n', X)
for i in range(Container_Num):
    for j in range(10):
...
Container_Placement_Based_On_MACS()
