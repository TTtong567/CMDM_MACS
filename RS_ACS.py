...
resource_total_EN = np.loadtxt('D:\ACS7\Resource_total_EN.txt', dtype='int', delimiter=',')
resource_Container = np.loadtxt('D:\ACS7\Resource_Container.txt', dtype='int', delimiter=',')
data_trans_Container = np.loadtxt('D:\ACS7\data_trans_Container.txt', dtype='int', delimiter=',')
Band = np.loadtxt('D:\ACS7\Band.txt', dtype='int', delimiter=',')
X = np.loadtxt('D:\ACS7\X.txt', dtype='int', delimiter=',')
print('边缘节点资源总量：\n', resource_total_EN)
print('容器资源需求：\n', resource_Container)
...
def Container_Selection():
    ...
    for i in list(set(Overload_Detection())):  # for i in overload_EN
        ...
        for j in range(len(ContainerList_total)):
            temp = random.choice(ContainerList_total)
            mig_Container.append(temp)
            resource_used_EN[i][0] -= resource_Container[temp][0]
            resource_used_EN[i][1] -= resource_Container[temp][1]
            resource_used_EN[i][2] -= resource_Container[temp][2]
            Y[temp][i] = 0
